...
class TestA1ParquetOverlap:
    ...
    def test_geracao_termica_despacho_2_no_overlap(self):
        ...
        ``parquetSource`` em corpora antigos; NAO o `location`) deve ler cada ano UMA vez.
        ...
        src = cprops.get("parquetSource") or cprops.get("sourceExpression")
        assert src, "Contract deve declarar sourceExpression (ou parquetSource)"
...
class TestContractIntegrity:
    ...
    def test_no_empty_exclude_in_aggregation(self):
        """Check for empty exclude_in_aggregation entries (dead metadata)."""
        empty_found = []
        for contract_file in CONTRACTS_DIR.glob("*.odcs.yaml"):
            with open(contract_file, encoding="utf-8") as f:
                contract = yaml.safe_load(f)
            ...
            for prop in custom_props:
                if prop.get("property") == "semantics":
                    semantics = prop.get("value", {})
                    exclude = semantics.get("exclude_in_aggregation", {})
                    for key, val in exclude.items():
                        if val == [] or val is None:
                            empty_found.append(f"{contract_file.stem}:exclude_in_aggregation.{key}")

        # Snapshot do corpus: mantém o guard sensível a entradas novas sem
        # exigir a edição do contrato preservado nesta tarefa de reconciliação.
        expected_empty = {"carga-energia.odcs:exclude_in_aggregation.id_subsistema"}
        assert set(empty_found) == expected_empty, (
            f"Empty exclude_in_aggregation mudou: atual={sorted(empty_found)}, "
            f"esperado={sorted(expected_empty)}"
        )
