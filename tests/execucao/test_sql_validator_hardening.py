...
# ------------------------------------------------------------------------------
# Teto de tamanho do SQL: recusa antes do parse, em todos os pontos de entrada.
# ------------------------------------------------------------------------------
def test_sql_acima_do_teto_e_recusado_antes_do_parse(monkeypatch):
    import time

    from mcp_tiago_dados_abertos.execucao import validator as V

    monkeypatch.setattr(V, "MAX_SQL_CHARS", 1000)
    v = _validador_isolado()
    sql = "SELECT " + ", ".join(str(i) for i in range(20000))
    t = time.perf_counter()
    ok, msg = v.validate(sql)
    assert not ok and "excede" in msg
    assert time.perf_counter() - t < 0.05, "a recusa por tamanho nao pode passar pelo parser"


def test_recusa_por_tamanho_tem_classe_de_erro_propria():
    from mcp_tiago_dados_abertos.execucao.validator import sql_tamanho_excedido
    from mcp_tiago_dados_abertos.infra import telemetry as T

    assert T.classe_erro(sql_tamanho_excedido("x" * 10_000_000)) == "tamanho"


def test_sql_grande_dentro_do_teto_passa():
    v = _validador_isolado()
    ok, msg = v.validate("SELECT " + ", ".join(str(i) for i in range(2000)))
    assert ok, msg


def test_executar_sql_raw_recusa_sql_gigante_sem_parse(monkeypatch):
    import asyncio
    import time
    ...
    monkeypatch.setattr(V, "MAX_SQL_CHARS", 1000)
    sql = "SELECT " + ", ".join(str(i) for i in range(50000))
    t = time.perf_counter()
    ok, msg, *_ = asyncio.run(db.executar_sql_raw(sql, limit=1, offset=0))
    assert not ok and "excede" in msg
...
# ------------------------------------------------------------------------------
# Cobertura das recusas de seguranca que nao tinham teste (08/09/2026).
# ------------------------------------------------------------------------------


class TestMultipleStatementsViaParser:
    """Exatamente 1 comando SQL permitido — via parser, nao so regex."""

    def test_dois_selects_separados_por_ponto_virgula(self):
        v = _validador_isolado()
        ok, msg = v.is_safe("SELECT 1; SELECT 2")
        assert not ok
        assert "Exatamente 1 comando" in msg or "Multiplos" in msg

    def test_zero_statements_apos_parse(self):
        v = _validador_isolado()
        # Comentario puro: sqlglot parseia como zero statements
        ok, msg = v.is_safe("-- apenas comentario")
        assert not ok
        assert "1 comando" in msg or "Exatamente" in msg or "sintaxe" in msg.lower()


class TestHttpsHostAllowlistEdgeCases:
    """_https_host_allowed return False — casos de borda."""

    @pytest.mark.parametrize(
        "sql,desc",
        [
            # Host com caracteres proibidos
            ("SELECT * FROM read_json('https://user@evil.com/x.json')", "@ in authority"),
            ("SELECT * FROM read_json('https://evil.com?q=x/x.json')", "? in path pretending host"),
            ("SELECT * FROM read_json('https://evil.com#frag/x.json')", "# in path"),
            # Host vazio
            ("SELECT * FROM read_json('https:///path/x.json')", "empty host"),
            # Subdominio squat (ons.org.br.evil.com)
            ("SELECT * FROM read_json('https://dados.ons.org.br.evil.com/x.json')", "subdomain squat"),
            # IPv6 (nao permitido)
            ("SELECT * FROM read_json('https://[::1]:8080/x.json')", "IPv6 localhost"),
        ],
    )
    def test_https_host_edge_cases_blocked(self, validator, sql, desc):
        ok, msg = validator.is_safe(sql)
        assert not ok, f"[{desc}] should be blocked: {sql}"


class TestPragmaIntrospection:
    """Funcao bloqueada (introspecao via pragma)."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "SELECT * FROM pragma_show('SELECT 1')",
        ],
    )
    def test_pragma_functions_blocked(self, validator, sql):
        ok, msg = validator.is_safe(sql)
        assert not ok, f"should block pragma function: {sql}"
        assert "bloqueada" in msg.lower() or "blocked" in msg.lower()


class TestSystemFunctionAccess:
    """Acesso a funcao de sistema bloqueado (table-valued functions de sistema)."""
    ...
    def test_system_functions_blocked(self, validator, sql):
        ok, msg = validator.is_safe(sql)
        assert not ok, f"should block system function: {sql}"


class TestReaderWithoutPath:
    """Leitor sem caminho — forma invalida de read_parquet()."""

    def test_read_parquet_no_args(self):
        v = _validador_isolado()
        # sqlglot pode nao parsear isso, mas se parsear, deve recusar
        ok, msg = v.is_safe("SELECT * FROM read_parquet()")
        # Pode falhar por sintaxe ou por falta de caminho — ambos sao recusa valida
        assert not ok

    def test_read_parquet_null_arg(self):
        v = _validador_isolado()
        ok, msg = v.is_safe("SELECT * FROM read_parquet(NULL)")
        assert not ok


class TestNonAllowedReaders:
    """Leitor nao permitido — leitores fora da allowlist."""
    ...
    def test_non_allowed_reader_blocked(self, validator, sql):
        ok, msg = validator.is_safe(sql)
        # Pode ser bloqueado por _check_non_allowed_readers ou por _check_blocked_functions
        # — ambos sao caminhos validos de recusa
        assert not ok, f"should block non-allowed reader: {sql}"


class TestS3BucketAllowlistExplicit:
    """Host HTTPS nao autorizado / Bucket S3 nao autorizado — mensagens explicitas."""

    def test_s3_bucket_not_allowed_message(self):
        v = _validador_isolado()
        ok, msg = v.is_safe("SELECT * FROM read_parquet('s3://outro-bucket/x.parquet')")
        assert not ok
        assert "nao autorizado" in msg.lower() or "not authorized" in msg.lower()

    def test_https_host_not_allowed_message(self):
        v = _validador_isolado()
        ok, msg = v.is_safe("SELECT * FROM read_json('https://evil.com/x.json')")
        assert not ok
        assert "nao autorizado" in msg.lower() or "host" in msg.lower()
