...
def test_pattern_failure_falls_back_to_build_from_metrics():
    # Pattern de comparacao do carga-energia falha (GROUP BY agrega) -> o fallback
    # deterministico (build_from_metrics) deve executar e retornar dados.
    import asyncio

    from mcp_tiago_dados_abertos.catalogo.catalog import find, load_all_contracts
    from mcp_tiago_dados_abertos.motor import semantics_engine as se

    p = se.plan(
        "compare a carga de energia do norte e do nordeste em 2024", find(load_all_contracts(), "carga-energia")
    )
    assert p.fallback_sql, "fallback ausente"
    ok, out = asyncio.run(db.executar_sql(p.fallback_sql, limit=5))
    if not ok and "LocalFileSystem" not in out:
        pytest.skip(f"S3/exec indisponivel: {out[:80]}")
    assert ok, f"fallback nao executou: {out[:150]}"
