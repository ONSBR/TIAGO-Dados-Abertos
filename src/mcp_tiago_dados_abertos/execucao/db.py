...
from mcp_tiago_dados_abertos.execucao.validator import SqlValidator, sql_tamanho_excedido
...
try:
    ...
    # particoes via boto3 (execucao/pruning.py) tambem e anonima. Validado ao vivo: leitura
...
async def executar_sql(
    ...
    TODOS os caminhos atravessam (as tools e os scripts), entao a telemetria
...
async def executar_sql_raw(
    ...
    Usada pela tool executar_sql, que monta o result-schema a partir de columns/rows.
...
async def _executar_sql_nucleo(
    ...
    # Teto de tamanho antes de tudo: e a unica checagem que nao depende de parse.
    msg_tam = sql_tamanho_excedido(sql)
    if msg_tam:
        return False, msg_tam, [], [], False
    ...
    # Roda no executor: o parse custa tempo proporcional ao texto e nao pode ocupar
    # o event loop. _run_sync revalida (barato).
    ok_v, msg_v = await _in_exec(validator.validate, sql)
    if not ok_v:
