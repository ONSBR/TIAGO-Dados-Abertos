...
from mcp_tiago_dados_abertos.infra.config import MAX_SQL_CHARS
...
def sql_tamanho_excedido(sql: str) -> Optional[str]:
    """Teto de tamanho, a checagem mais barata e por isso a primeira. Chamada antes de
    qualquer parse (validador, poda, rota), em todos os pontos de entrada do SQL."""
    n = len(sql or "")
    if n > MAX_SQL_CHARS:
        return f"SQL com {n} caracteres excede o maximo de {MAX_SQL_CHARS}. Reduza a consulta."
    return None
...
class SqlValidator:
    ...
    def validate(self, sql: str) -> tuple[bool, str]:
        """Validacao completa: tamanho + seguranca + sintaxe."""
        msg_tam = sql_tamanho_excedido(sql)
        if msg_tam:
            return False, msg_tam
