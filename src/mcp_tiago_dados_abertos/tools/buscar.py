...
from mcp_tiago_dados_abertos.catalogo.catalog import rank_datasets
from mcp_tiago_dados_abertos.catalogo.ranking import _PERIODO_WORDS, _STOPWORDS_CONVERSA
from mcp_tiago_dados_abertos.tools.descrever_dataset import _fonte_legivel
...
def termos_da_pergunta(tema: str) -> set[str]:
    ...
    return {t for t in toks
            if not t.isdigit() and t not in _PERIODO_WORDS and t not in _MESES and t not in _STOPWORDS_CONVERSA}
...
async def buscar_dataset(tema: str, *, catalog: dict, orgao: str | None = None) -> str:
    ...
    for rank_pos, (score, name, meta) in enumerate(ranked, 1):
        ...
        linhas.append(f"**parquet_source:** `{_fonte_legivel(meta.get('parquet_source', 'N/A'))}`")
