...
from mcp_tiago_dados_abertos.catalogo.contracts import find, format_fewshots, format_schema
...
def fonte_legivel(source: str) -> str:
...
def _format_semantics(meta: dict) -> str:
    ...
    if aliases:
        # value_aliases lista conceitos, e nem todo conceito e' coluna. Em quatro
        # contratos 'fonte' e' um PIVO de varias colunas de geracao: anunciar como
        # filtravel faz o modelo escrever WHERE fonte = 'eolica' e receber Binder
        # Error. semantics.dimension_columns diz qual e qual.
        dim_cols = sem.get("dimension_columns") or {}
        colunas_reais = {str(c.get("name", "")).lower() for c in (meta.get("columns") or [])}
        filtraveis, pivos = [], []
        for nome in aliases:
            alvo = dim_cols.get(nome)
            if isinstance(alvo, str) and alvo.startswith("_pivot("):
                pivos.append((nome, alvo[len("_pivot("):-1] if alvo.endswith(")") else alvo))
            elif (isinstance(alvo, str) and alvo.lower() in colunas_reais) or \
                    str(nome).lower() in colunas_reais or not dim_cols:
                filtraveis.append(nome)
            elif alvo:
                filtraveis.append(nome)
        if filtraveis:
            lines.append(f"**Dimensoes filtraveis (com aliases):** {', '.join(filtraveis)}")
        for nome, cols in pivos:
            lines.append(
                f"**Atencao:** `{nome}` NAO e' coluna; e' a leitura conjunta de {cols}. "
                f"Nao use WHERE {nome} = ...; para separar por {nome}, empilhe essas colunas."
            )
...
async def descrever_dataset(nome_dataset: str, *, catalog: dict) -> str:
    ...
    parts = [
        f"# [ONS] {meta['name']}",
        f"**parquet_source:** `{fonte_legivel(meta.get('parquet_source', 'N/A'))}`",
