...
from mcp_tiago_dados_abertos.resposta.columns import _RELATIVA_COL_RE, _TEMPORAL_COL_RE, _coluna_iso, _sem_acento
...
def _somavel(metrica) -> bool:
    try:
        from mcp_tiago_dados_abertos.resposta.additivity import metrica_somavel

        return metrica_somavel(metrica) is True
    except Exception:
        return False
...
def _marca_colunas_vazias(cols_rs: list, rows) -> None:
    """Marca a coluna que voltou INTEIRAMENTE nula no resultado desta pagina.

    A fonte publica acrescenta e renomeia colunas ao longo da serie, e todo
    parquet_source usa union_by_name=true: quando o arquivo de um periodo nao tem
    a coluna, o DuckDB preenche com NULL em vez de acusar erro. A consulta entao
    devolve um numero plausivel e errado, porque SUM/AVG ignoram nulo e o
    denominador encolhe sem aviso.

    A checagem e' feita no dado devolvido, nao em metadado: pega tambem a coluna
    que mudou depois da ultima auditoria. Marca so' o caso 100% nulo, que e'
    inequivoco; nulo parcial e' comum em dado real e viraria ruido.
    """
    if not rows:
        return
    for i, c in enumerate(cols_rs):
        vistos = vazios = 0
        for r in rows:
            if not isinstance(r, (list, tuple)) or len(r) <= i:
                continue
            vistos += 1
            vazios += r[i] is None
        if vistos and vazios == vistos:
            c["all_null"] = True
            c["note"] = ("coluna sem nenhum valor nas linhas retornadas; se o recorte "
                         "cobre periodos antigos, ela pode nao existir nos arquivos "
                         "daquele periodo — confira em descrever_dataset antes de "
                         "somar, calcular media ou afirmar ausencia do fenomeno")
...
def result_schema(sql, columns, metas, *, rota=None, rows=None) -> dict:
    ...
    # Fora do try acima: vale para a tipagem completa E para o fallback desconhecido,
    # e nunca deixa de rodar por causa de uma falha na tipagem.
    try:
        _marca_colunas_vazias(out["columns"], rows)
    except Exception:
        pass
    return out
