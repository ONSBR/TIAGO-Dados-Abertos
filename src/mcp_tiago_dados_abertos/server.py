...
from mcp_tiago_dados_abertos.resposta.context import temporal_footer
...
# version= e' obrigatorio: sem ele o FastMCP anuncia a VERSAO DELE no handshake
# (serverInfo), e o cliente MCP exibe a versao da biblioteca no lugar da nossa.
mcp = FastMCP("MCP TIAGO Dados Abertos - ONS", version=VERSION)

# ── Tools ─────────────────────────────────────────────────────────────────────

from mcp_tiago_dados_abertos.infra.toolaudit import log_tool_call
from mcp_tiago_dados_abertos.tools.buscar_dataset import buscar_dataset as _buscar
...
async def executar_sql(sql_query: str, limit: int = 100, offset: int = 0) -> str:
    ...
    from mcp_tiago_dados_abertos.execucao.pruning import prune_user_sql
    ...
    from mcp_tiago_dados_abertos.resposta.unit_advisor import aviso_unidade

    # Antes da poda e da rota, que tambem fazem parse do SQL.
    msg_tam = sql_tamanho_excedido(sql_query)
    if msg_tam:
        # Mesmo evento `sql` do db.py, para a recusa entrar nos contadores existentes.
        try:
            from mcp_tiago_dados_abertos.infra.telemetry import classe_erro, evento

            evento(evt="sql", ok=False, latency_ms=0, resp_bytes=len(msg_tam), classe_erro=classe_erro(msg_tam))
        except Exception:
            pass
        return f"[ERRO] {msg_tam}"
    _ensure_catalog()
