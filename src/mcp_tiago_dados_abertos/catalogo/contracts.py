...
def yaml_manifest(base: Path) -> str:
    """sha256 de (caminho relativo, tamanho, sha256 do conteudo) de todo *.odcs.yaml sob base.

    Usada pelo build do cache (scripts/build_contracts_cache.py) e pela validacao no boot.
    E por conteudo, nao por mtime: ao exportar uma camada de imagem o Docker trunca o
    mtime para segundos inteiros, e um manifesto por mtime_ns calculado no build nunca
    mais batia em runtime. O cache da imagem era rejeitado em toda carga.
    """
    itens = []
    for p in sorted(base.rglob("*.odcs.yaml")):
        dados = p.read_bytes()
        itens.append((str(p.relative_to(base)).replace(chr(92), "/"), len(dados), hashlib.sha256(dados).hexdigest()))
...
def _cache_valido(path: Path) -> bool:
    ...
    - se "yaml_manifest" presente: os YAML em disco tem que ser os do build (nome, tamanho, conteudo)
    ...
    try:
        ...
        if "yaml_manifest" in raw:
            if raw["yaml_manifest"] != yaml_manifest(Path(path).parent):
                return False
...
def load_all_contracts() -> dict:
        ...
        if True:
            ...
            try:
                ...
                for schema_obj in data.get("schema", []):
                    for prop in schema_obj.get("properties", []):
                        ...
                        for cp in prop.get("customProperties", []):
                            ...
                            elif cp.get("property") == "availableFrom":
                                col["available_from"] = cp.get("value")
                            elif cp.get("property") == "coverageNote":
                                col["coverage_note"] = cp.get("value")
                ...
                catalog[key] = {
                    ...
                    "name": name,
                    "s3_location": s3,
                    # read_parquet(...) embrulhado; fallback = location s3 crua.
                    "parquet_source": custom.get("sourceExpression") or s3,
                    ...
                    "rag_context": custom.get("aiContext", ""),
                    ...
                    # Fim ausente = serie ativa, "ate o presente" (ver _format_cobertura).
                    "period_start": custom.get("temporalCoverageStart", ""),
                    "period_end": custom.get("temporalCoverageEnd", ""),
                    ...
                    # value_aliases, aggregation_modes, sql_patterns).
                    # Fonte de verdade para o result-schema e a aditividade.
    ...
    # measure_casts.py (medido dos examples reais, nao por nome).
...
def format_schema(meta: dict) -> str:
    ...
    for col in cols:
        ...
        # Cobertura parcial: a fonte acrescentou esta coluna no meio da serie. Como a
        # leitura usa union_by_name, periodo anterior volta NULL e nao erro, e AVG/SUM
        # ignoram nulo — a media da serie sai da media so' do trecho que tem a coluna.
        # Por isso o aviso vem junto do nome, antes de o modelo escrever o SQL.
        if col.get("available_from"):
            lines.append(f"      ATENCAO_disponivel_a_partir_de: {col['available_from']}")
        if col.get("coverage_note"):
            lines.append(f"      ATENCAO_cobertura: {col['coverage_note']}")
        if col.get("aliases"):
