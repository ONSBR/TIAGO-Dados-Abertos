...
Era a segunda metade de contracts.py; carregar contrato e ranquear pergunta sao
...
# Sinal lexical (BM25, retrieval.py): so o bonus DISCRETO de posicao. Estar entre os tres
# primeiros do BM25 vale 18/12/6 pontos. O peso continuo do escore, o gate de conviccao e o
# teto de tags que existiam para o TF-IDF foram medidos em 44 perguntas reais fora da
# calibracao (05/09/2026) e pioravam ou nao mudavam nada; sairam.
# Ate o 5o colocado do BM25, com bonus decrescente (25/20/15/10/5). O corte no 3o era um
# degrau: um dataset em 4o lugar recebia zero, mesmo com escore proximo do 3o (foi o caso
# de 'evolucao da MMGD', BM25 em 4o com 0,51). Medido em 07/09/2026: top-3 85,1 -> 85,9
# (p = 0,04); o top-1 nao mudou fora do conjunto que escolheu a configuracao.
_SEM_RANK_MULT = 5
_SEM_RANK_TOPN = 5
...
# Dataset de referencia da familia (semantics.family + semantics.family_default no contrato).
# Irmaos de uma familia (carga diaria/horaria/mensal, EAR por subsistema/reservatorio...) so
# diferem no grao; quando a pergunta NAO diz o grao, o contrato marcado como referencia da
# familia recebe o bonus. Pergunta com grao explicito nao e' tocada. Vale so para dataset que ja
# pontuou pelo assunto (score > 0).
_FAMILIA_PESO = 9
# Vocabulario da MEDIDA (semantics.measure_terms no contrato): as palavras que nomeiam o que o
# dataset publica (corte/curtailment, geracao, disponibilidade, capacidade instalada, CVU...).
# Diferente de tag: lista curta e fechada por medida, e conta UMA vez por contrato, entao
# nao ha como "encher" o campo para pesar mais. Separa irmaos de ativo igual e medida
# diferente (corte x geracao da mesma usina). Vale so para dataset que ja pontuou (score > 0).
_MEDIDA_PESO = 6
_PALAVRAS_DE_GRAO = frozenset({"hora", "horas", "horario", "horaria", "horarios", "horarias", "semihorario",
                              "dia", "dias", "diario", "diaria", "diarios", "diarias", "ontem", "anteontem",
                              "semana", "semanas", "semanal", "mes", "meses", "mensal", "mensais",
                              "ano", "anos", "anual"})
...
def rank_datasets(catalog: dict, pergunta: str, top_n: int = 3, orgao: Optional[str] = None) -> list:
    ...
    tokens = tokens - stopwords - STOPWORDS_CONVERSA
    ...
    tem_periodo = bool(tokens_crus & PERIODO_WORDS) or bool(_ANO_RE.search(pergunta))
    ...
    for key, meta in catalog.items():
        ...
        # Referencia da familia quando a pergunta nao diz o grao (ver _FAMILIA_PESO).
        if score > 0 and _FAMILIA_PESO and (meta.get("semantics") or {}).get("family_default") \
                and not (tokens_crus & _PALAVRAS_DE_GRAO):
            score += _FAMILIA_PESO
        # Medida citada na pergunta x medida do contrato (ver _MEDIDA_PESO).
        termos_medida = (meta.get("semantics") or {}).get("measure_terms") or []
        if score > 0 and _MEDIDA_PESO and termos_medida:
            pn = " " + _normalize(pergunta) + " "
            if any((" " + _normalize(str(tm)) + " ") in pn for tm in termos_medida):
                score += _MEDIDA_PESO
    ...
    top_sem = {n: i for i, (s, n, _) in enumerate(sem_results[:_SEM_RANK_TOPN])}
    combined = {}
    for key in set(kw_scores.keys()) | set(top_sem.keys()):
        ...
        combined[key] = kw_scores.get(key, 0) + rank_boost
