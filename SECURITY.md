...
- **A checagem falha fechada.** O caminho de um leitor tem que ser literal de string simples
  ...
  `FROM 'caminho'` sem função de leitor são recusados. A autoridade da URL não pode conter
  `@ # ? \ %`.
...
- **Recursos limitados.** Tempo por consulta (`MCP_QUERY_TIMEOUT_S`), memória do DuckDB
  (`MCP_DUCKDB_MEMORY_LIMIT`), tamanho de página (`MCP_MAX_PAGE_LIMIT`) e tamanho do SQL
  (`MCP_MAX_SQL_CHARS`) têm teto. Literais numéricos gigantes em funções geradoras
  (`repeat`, `range`, `generate_series`) são recusados antes de executar.
