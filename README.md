...
- A busca é TF-IDF e palavra-chave, sem embedding neural. Tema fora do vocabulário dos
  contratos devolve poucos candidatos ou nenhum; sinônimos entram pelo campo `vectorTags`.
- Datasets grandes dependem da poda de partições por ano, declarada no contrato como
  `partitionPruning`, que só existe onde foi validada por medição.
